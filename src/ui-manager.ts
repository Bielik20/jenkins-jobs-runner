...
export class UiManager {
    ...
    this.batchMaxNameWidth = batchDescriptors
      .map(
        (batchDescriptor: JobBatchDescriptor) =>
          batchDescriptor.displayName.length,
      )
      .reduce((prev, curr) => (prev > curr ? prev : curr), 0);

    this.jobMaxNameWidth = batchDescriptors
      .map(
        (batchDescriptor: JobBatchDescriptor) => batchDescriptor.jobDescriptor,
      )
  ...
  printBatchFooter(results: JobDone[]): void {
    ...
    process.stdout.write(
      ansiEscapes.cursorDown(results.length + 1) + ansiEscapes.cursorLeft,
    );
  ...
  private createBar(jobDescriptor: JobDescriptor): ProgressBar {
    const fillLength: number =
      this.jobMaxNameWidth - jobDescriptor.displayName.length;
  ...
  private updateBar(bar: ProgressBar, response: JobResponse): void {
    ...
    if (isJobDone(response)) {
      const symbol: string =
        response.status === 'SUCCESS' ? logSymbols.success : logSymbols.error;
      const link: string = response.url;
